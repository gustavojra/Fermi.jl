...
function RCCSDpT(ccsd::RCCSD, moints::IntegralHelper{T,E,O}, Alg::ijk2) where {T<:AbstractFloat, E<:AbstractERI, O<:AbstractRestrictedOrbitals}
    ...
    # Streaming worker-pool: each task owns its own local W/V/tV buffers and
    # energy accumulator, pulling `i` values off a shared queue, instead of
    # indexing per-thread arrays by Threads.threadid() (unsafe under task
    # migration). The workload per `i` is triangular, so a dynamic queue also
    # balances load better than a static split across threads.
